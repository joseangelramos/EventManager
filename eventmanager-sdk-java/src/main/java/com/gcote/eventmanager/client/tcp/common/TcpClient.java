...
package com.gcote.eventmanager.client.tcp.common;
...
public abstract class TcpClient implements Closeable {
    ...
    protected synchronized void open(SimpleChannelInboundHandler<Package> handler) throws Exception {
        ...
        channel = f.channel();
        logger.info("conectado|local={}:{}|server={}", localAddress.getAddress().getHostAddress(), localAddress.getPort(), host + ":" + port);
    }
    ...
    public void close() throws IOException {
        try {
            ...
            logger.warn("fallo cerrar cliente tcp.|remote address={}", channel.remoteAddress(), e);
        }
    ...
    protected void send(Package msg) throws Exception {
        if (channel.isWritable()) {
            channel.writeAndFlush(msg).addListener((ChannelFutureListener) future -> {
                if (!future.isSuccess()) {
                    logger.warn("fallo envio de mensaje cliente tcp", future.isSuccess(), future.cause());
                }
            });
        } else {
    ...
    protected Package io(Package msg, long timeout) throws Exception {
        ...
        if (!contexts.contains(c)) {
            ...
            logger.info("llave duplicada : {}", key);
        }
        send(msg);
        if (!c.getLatch().await(timeout, TimeUnit.MILLISECONDS))
            throw new TimeoutException("timeout de operacion, context.key=" + c.getKey());
    ...
    private ChannelDuplexHandler newExceptionHandler() {
        return new ChannelDuplexHandler() {
            ...
            public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
                logger.info("exceptionCaught, cerrando conexion.|remote address={}",ctx.channel().remoteAddress(),cause);
                ctx.close();
            }
        };
    }
