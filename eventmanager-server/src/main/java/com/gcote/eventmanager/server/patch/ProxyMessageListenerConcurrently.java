...
package com.gcote.eventmanager.server.patch;
...
public abstract class ProxyMessageListenerConcurrently implements MessageListenerConcurrently {
    ...
    public ConsumeConcurrentlyStatus consumeMessage(final List<MessageExt> msgs,
        ...
        try {
            ProxyConsumeConcurrentlyContext proxyConsumeConcurrentlyContext = (ProxyConsumeConcurrentlyContext) context;
            ProxyConsumeConcurrentlyStatus proxyConsumeStatus = handleMessage(msg, proxyConsumeConcurrentlyContext);
            try {
                switch (proxyConsumeStatus) {
                    case CONSUME_SUCCESS:
                        return ConsumeConcurrentlyStatus.CONSUME_SUCCESS;
                    case RECONSUME_LATER:
                        return ConsumeConcurrentlyStatus.RECONSUME_LATER;
                    case CONSUME_FINISH:
                        proxyConsumeConcurrentlyContext.setManualAck(true);
                        return ConsumeConcurrentlyStatus.CONSUME_SUCCESS;
                }
            } catch (Throwable e) {
